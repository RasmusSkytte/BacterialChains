...
int main(int argc, char** argv){
    ...
    std::vector<numtype> T_compress = {  1.00,   1.5,  2.25,  3.35,  5.00,  7.50,  11.2};
    ...
    for (int n = 0; n < N.size(); n++) {
        for (int r = 0; r < repeats[n]; r++) {
            ...
            // Set the path
            string pathName = "SphericalColony";
            ...
            // Set the cells to be clumped
            s.SphericalColony(10);
            s.CellAttractiveParameter(0);

            // Set the angle between cells
            s.CellBendingAngle(3 / 2 * M_PIl);

            // Let the cells contract into colony
            s.CellLength(0.1);
            s.TimeStep(dT_relax);
            s.Run(T_compress[n]);

            // Then let the colony relax
            s.CellLength(Ld);
            s.CellRadius(R);
            s.SphericalColony(0.0);
            s.Relax();
